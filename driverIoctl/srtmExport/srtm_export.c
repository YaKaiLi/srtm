...
asmlinkage int srtm_pull_image(char *ConfigJSONKernel)
{
...
asmlinkage static int srtm_run_container(char *ConfigJSONKernel)
{
	// char **uintptrConfigJSON, int *uintptrConfigJSONLen
	// di、si、dx、r10、r8、r9
	// int retTmp = 456;
...
static int __init init_srtm_export(void)
{
	printk("SRTM export is starting...\n");
	return 0;
}

/*出口函数，卸载模块*/
static void __exit exit_srtm_export(void)
{
	printk("SRTM export exit....\n");
}

EXPORT_SYMBOL(srtm_pull_image);
EXPORT_SYMBOL(srtm_run_container);

module_init(init_srtm_export);
module_exit(exit_srtm_export);
MODULE_LICENSE("GPL");